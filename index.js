...
(function () {
    ...
    function openModal() {
        ...
        if (!overlay) {
            overlay = buildModal();
            // body에 transform이 걸려있으면 position:fixed가 깨짐 (MovingUI 이슈)
            // html 요소에 붙이면 항상 viewport 기준으로 고정됨
    ...
    // ★ v9 핵심 수정 ★
    // ST가 jQuery 이벤트 위임을 쓰기 때문에
    // document capture의 stopImmediatePropagation으로는 막을 수 없음.
    // window capture + pointerdown을 써야 jQuery보다 먼저 실행됨.
    // pointerdown에서 preventDefault()를 호출하면 이후 click 이벤트 자체가
    // 생성되지 않아 ST의 click 핸들러가 아예 실행되지 않음.

    function isPersonaBtn(el) {
        return !!el?.closest(
    ...
    window.addEventListener('pointerdown', e => {
        if (!isPersonaBtn(e.target) || !isEnabled()) return;
        e.preventDefault();
        e.stopImmediatePropagation();
        openModal();
    }, { capture: true });

    // pointerdown이 안 먹히는 환경을 위한 click 백업
