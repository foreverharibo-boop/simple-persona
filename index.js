...
//
// ★ v7: 진단 모드 ★
// 콘솔을 볼 수 없는 환경이라, 확장이 실제로 로드됐는지
// 화면에 초록색 배너를 5초간 띄워서 눈으로 확인 가능하게 함.
//   - 배너가 보인다  → 확장 로드 OK. 문제는 UI 로직에 있음.
//   - 배너가 안 보인다 → 확장 자체가 로드 안 됨 (경로/manifest/캐시 문제).
// 이 한 가지만 알면 원인을 절반으로 좁힐 수 있음.
...
// ─── ① 로드 확인용 배너 (최상단, import 직후 즉시 실행) ────────────────────
(function showLoadBanner() {
    const b = document.createElement('div');
    b.textContent = '✅ Simple Persona 로드됨 (v7)';
    b.style.cssText = `
        position:fixed; top:0; left:0; right:0; z-index:2147483647;
        background:#16a34a; color:#fff; font-size:14px; font-weight:700;
        padding:10px; text-align:center; font-family:sans-serif;
    `;
    const put = () => document.body?.appendChild(b);
