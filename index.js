...
(function () {
    ...
    function openCreateDialog() {
        ...
        dlg.querySelector('#sp-new-confirm').addEventListener('click', async () => {
            ...
            // ★ MutationObserver로 ST가 실제 등록한 파일명을 감지
            // ST는 업로드 시 타임스탬프를 붙여 파일명을 바꾸므로
            // selectedFile.name으로 등록하면 안 됨 — 새로 추가된 avatar-container를 감지해야 함
            const block = document.getElementById('user_avatar_block');
            const existingFiles = new Set(
                [...document.querySelectorAll('#user_avatar_block .avatar-container')]
                    .map(el => el.dataset.avatarId)
            );

            let registered = false;
            const obs = new MutationObserver(() => {
                if (registered) return;
                const newCard = [...document.querySelectorAll('#user_avatar_block .avatar-container')]
                    .find(el => !existingFiles.has(el.dataset.avatarId) && el.dataset.avatarId);
                if (newCard) {
                    registered = true;
                    obs.disconnect();
                    const newFile = newCard.dataset.avatarId;
            ...
            if (block) obs.observe(block, { childList: true, subtree: true, attributes: true, attributeFilter: ['data-avatar-id'] });
