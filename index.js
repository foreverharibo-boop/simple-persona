...
(function () {
    ...
    function getPersonaMemo(f) {
        // 카드 서브텍스트: ch_additional_info (짧은 메모) 우선, 없으면 설명 첫 줄
        const domMemo = document.querySelector(`#user_avatar_block [data-avatar-id="${CSS.escape(f)}"] .ch_additional_info`)?.textContent?.trim();
        if (domMemo) return domMemo;
        const desc = power_user.persona_descriptions?.[f]?.description ?? '';
        // 설명에서 의미있는 첫 줄 추출 (마크다운 헤더/빈줄 제외)
        for (const line of desc.split('\n')) {
            const t = line.trim();
            if (t && !t.startsWith('#') && !t.startsWith('**') && !t.startsWith('---')) return t.slice(0, 50);
        }
        return '';
    }
    function getPersonaFullDesc(f) {
    ...
    function openEditDialog(file) {
        ...
        const desc = getPersonaFullDesc(file);
    ...
    function confirmDelete(file) {
        ...
        conf.querySelector('#sp-conf-ok').addEventListener('click', async () => {
            ...
            if (!deleted) {
                try {
                    const res = await fetch(`/deleteUserAvatar?avatar=${encodeURIComponent(file)}`);
                    deleted = res.ok;
                } catch {}
            }
            // 항상 메타데이터 정리
            ...
            const s = getSettings();
            if (s.favorites?.[file]) delete s.favorites[file];
            if (power_user.default_persona === file) delete power_user.default_persona;
            saveSettingsDebounced();
            if (getCurrentPersona() === file) await setUserAvatar(null);
    ...
    function openModal() {
        ...
        if (!overlay) {
            ...
            // ★ 인라인 스타일로 강제 고정 (MovingUI body transform 영향 차단)
