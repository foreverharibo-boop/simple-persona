...
(function () {
    ...
    /** 카드/서브텍스트용: DOM의 짧은 메모(ch_additional_info) 우선.
     *  없으면 설명에서 의미있는 첫 줄만 짧게 추출 (마크다운 헤더 등 제외) */
    function getPersonaMemo(f) {
        ...
        for (const line of desc.split('\n')) {
            const t = line.trim();
            if (t && !t.startsWith('#') && !t.startsWith('**') && !t.startsWith('---') && !t.startsWith('===')) return t.slice(0, 50);
        }
        return '';
    }
    /** 현재 페르소나 배너 전용: 순수 메모(ch_additional_info)만. 없으면 빈 문자열. */
    function getPersonaShortMemo(f) {
    ...
    function updateCurrentBanner() {
        ...
        if (subEl) subEl.textContent = f ? `메모: ${getPersonaShortMemo(f) || '없음'}` : '현재 선택된 페르소나';
    ...
    function openCreateDialog() {
        ...
        dlg.querySelector('#sp-new-confirm').addEventListener('click', async () => {
            ...
            const pollTimer = setInterval(async () => {
                ...
                if (!newFile) {
                    const current = await getUserAvatars(true).catch(() => []);
