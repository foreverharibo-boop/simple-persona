...
(function () {
    ...
    /** file -> 서브텍스트(설명 첫 줄). power_user.persona_descriptions[file].description */
    ...
    /** Extensions 메뉴(설정 서랍)에 심플 페르소나 항목 + 활성화 체크박스 추가 */
    async function injectSettingsUI() {
        const container = document.getElementById('extensions_settings2')
            ?? document.getElementById('extensions_settings');
        if (!container || document.getElementById('sp-settings-block')) return;
        ...
        block.innerHTML = `
            ...
            <div class="inline-drawer-content" style="display:none;">
                <label class="checkbox_label" style="display:flex;align-items:center;gap:8px;margin:6px 0;">
                    <input id="sp-enabled-toggle" type="checkbox" ${settings.enabled ? 'checked' : ''} />
                    <span>확장 활성화 (페르소나 버튼 클릭 시 심플 페르소나 열기)</span>
                </label>
            </div>`;
        container.appendChild(block);

        // ST의 서랍 토글 공통 동작(열고 닫기)
    ...
    function init() {
        getSettings(); // 설정 기본값 보장
        applyEnabledState();
        injectSettingsUI();

        if (!hookPersonaButton()) {
            // ST가 아직 UI를 다 그리기 전이면 잠깐 기다렸다가 재시도
